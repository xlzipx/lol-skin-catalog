...
## What changed since 1.11.3

- **The rarity gems are Riot's own artwork now.** Until this release the gems
  were shapes drawn to resemble the ones in the client. They are the real thing
  now, fetched from Community Dragon at run time like every other image.
- Riot publishes gems for the six tiers the client displays. The legacy Rare
  tier has real data but no artwork, so it keeps a drawn gem, resized to sit
  level with the others rather than looking undersized beside them.
- The drawn set stays in place as a fallback if the download does not happen.

No artwork is stored in the repository or bundled into the download — the gems
arrive alongside the splash art when you run the program.
...
```
SHA256  0904C87AD3AAE0854218FD875D17B835F497CA770AAF5EEDE394F2165E342247  LoL-Skin-Catalog.zip (Windows)
```
