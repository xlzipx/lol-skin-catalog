...
- **Fixed a half-buried page number on the roster page.** The champion table
  reaches the foot of that page, so the number underneath it showed only as a
  sliver of a digit. Front matter is unnumbered now: the cover and the roster
  carry no number, and numbering starts at 3 with the first page of skins.
...
Source code is below if you would rather run it with Python. Windows is what
this executable targets; on macOS run it from source.
...
5. The first run downloads several hundred images and takes a few minutes. The PDF opens by itself when it finishes, and the window closes on its own shortly after.
...
```
SHA256  4EFDD9B96579E876E4134408D1B1D9566D8E5137C0299F02A8AFB88703634C7C  LoL-Skin-Catalog.zip
```
