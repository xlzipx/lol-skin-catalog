...
## What changed since 1.9.0

- **The window closes by itself** a few seconds after a successful export, so
  you are not left with a console to dismiss. A failed run still waits for
  Enter — an error nobody gets to read is no better than a silent failure.
  `--keep-open` brings back the old behaviour.
- **Better footing off Windows.** On macOS the running client is now found
  through `ps`, the same way it is found on Windows, rather than relying on the
  lockfile alone. Fonts are looked for in the macOS and Linux font folders too,
  so headings keep their serif face, and the finished PDF opens with `open` or
  `xdg-open`.

A note on platforms: Windows is what the released executable targets and what
CI exercises end to end. macOS support is written to the documented paths but
has not been tested on a Mac. Linux has no native League client, so the client
cannot be discovered automatically there, though the rest of the tool runs.
...
```
SHA256  0CFB6BEA70165467C29E823C5BB0F2F0501F96D277D746C48B24504807AB1D83  LoL-Skin-Catalog.zip
```
